...
const Terminal: React.FC = () => {
  ...
  const exec = (cmd: string) => {
    ...
    const c = parts[0];
    const lines: string[] = [`nova@os:~$ ${cmd}`];
    ...
    switch (c) {
      ...
      case 'uname': lines.push('NOVA OS 3.0.0 (Secure Kernel) x86_64'); break;
      ...
      case 'neofetch':
        ...
        break;
      case '': break;
      default: lines.push(`nova-sh: ${c}: command not found`);
    }
    lines.push('');
  ...
  return (
    <div className="flex flex-col h-full bg-black/50 font-mono text-xs">
      <div className="flex-1 overflow-y-auto p-3">
        {history.map((line, i) => (
          <div key={i} className="whitespace-pre-wrap" style={{ color: line.startsWith('nova@') ? '#00ff88' : '#e0e0e0' }}>
            {line}
          </div>
        ))}
        <div ref={endRef} />
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2 p-2 border-t border-white/10">
        <span className="text-green-400">nova@os:~$</span>
